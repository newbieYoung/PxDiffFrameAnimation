...
onmessage = function(e) {
	var resource = e.data;
	var emptyImageData = resource.emptyImageData;
   	var imageData = resource.imageData;
   	var pxImageData = resource.pxImageData;
   	var position = resource.position;
   	var fillLength = resource.fillLength;
   	//还原
	var dataList = new DoublyLinkedList();
	for(var j=0;j<pxImageData.length;j++){
		dataList.push(pxImageData[j]);
	}
	//去除掉新增的所有255alpha值
	var tempNo = 0;
	var tempIndex = 0;
	var tempNode = dataList.getHead();
	while(tempIndex<fillLength-1){
		tempNo++;
		tempIndex++;
	...
	for(var j=position.length-4;j>=0;j=j-2){
		var start = position[j];
		var space = position[j+1];
	...
	while(node.next){
		if(node.value&&!(node.value instanceof Array)){
			if(imageData.data[no]===node.value){
				imageData.data[no] = 0;
			}else{
	...
	for(var j=0;j<imageData.data.length;j++){
		emptyImageData.data[j] = imageData.data[j];
	}
	postMessage(emptyImageData);
}